...
if LUASCRIPTS_REF then
    scriptsgb = gui.Groupbox(LUASCRIPTS_REF, "Scripts", 510, 308, 222, 292)
end
...
if scriptsgb then
g.scripts_list = gui.Listbox(scriptsgb, "scripts_helpers", 152, "Wallbang Helper", "Grenade Helper", "Sync")
...
-- Vote Bypass logic removed

function vote_on_event(ev)
	local name = nil
	pcall(function() name = ev:GetName() end)

	-- Vote logging only (VB bypass removed)
	...
	elseif name == "vote_passed" then
		if g.logs_vote and g.logs_vote:GetValue() then
			vote_log("Vote ended: PASSED")
		end

	elseif name == "vote_failed" then
		if g.logs_vote and g.logs_vote:GetValue() then
			vote_log("Vote ended: FAILED")
		end
	end
end
...
function on_draw()
	...
	-- GH is NOT downloaded/compiled inside osnova_aa anymore.
	if g.gr_enable then
		local gh_on = g.gr_enable:GetValue()
		if gh_on and not gh_was_enabled then
			gh_was_enabled = true
			local L = rawget(_G, "__OSNOVA")
			if L and L.load_gh then
				pcall(L.load_gh)
			else
				print("[osnova] loader GH api missing; reload via osnova_loader_gh_api.lua")
			end
		elseif not gh_on and gh_was_enabled then
			...
			if L and L.unload_gh then
				...
				if GH_mod and GH_mod.uninstall then pcall(GH_mod.uninstall) end
			end
		end
	end
